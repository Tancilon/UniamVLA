...
3. **Patch `starVLA/model/framework/VLM4A/UamVLA.py`** at TWO call sites that face the same missing-mask problem: `forward()` (lines 318-329) and `visualize_batch()` (around line 654). Both currently default missing masks to all-True, which crashes pose / recon heads that read `batch["image_target"]` / assert `batch["point_cloud"] in batch` unconditionally. Replacement convention: action head defaults to all-True (action is universal); non-universal heads (pose / recon / future) get an **all-False default mask** so they enter `compute_loss` / `visualize` and exit through the heads' built-in `not mask.any()` early-exit (returning `get_dummy_loss()` for forward — required for DeepSpeed ZeRO-2 alignment per `base.py:31` — and a no-op for visualize). Both call sites use a shared helper `_resolve_head_mask` so forward, visualize, and the framework test all exercise the same code path.
...
| G6 | `UamVLA.py:319–323` (forward()'s aux-head dispatch) **AND `UamVLA.py:~654` (`visualize_batch()`'s aux-head dispatch)** | Both call sites do `mask = batch_dict.get(f"{name}_mask")` and fall back to `torch.ones(...)` (all-True) when missing. When every sample lacks an aux field, the collator drops both the field and its mask key (see `collator_helpers.py:93-94`); pose / recon heads then crash on `batch["image_target"]` (`recon_head.py:121` / `recon_head.py:179`) or `assert "point_cloud" in batch` (`pose_head.py:165` / `pose_head.py:367`). The forward path is fatal; the visualize path is caught + warned at `train_starvla.py:374` but causes visualization to randomly fail mid-training. | Action head: missing mask → all-True (correct, action is universal). Pose / recon / future: missing mask → **all-False default**, head's `compute_loss` / `visualize` early-exits via `not mask.any()` and returns `get_dummy_loss()` (forward) or a no-op (visualize). The dummy-loss path preserves DeepSpeed ZeRO-2 all-reduce shape across ranks (see `base.py:31`). Both call sites use a shared `_resolve_head_mask` helper. |
| G7 | `tests/test_uamvla_calvin_pipeline_local.py` | All 8 existing tests build fixtures with every aux field present. None covers a "partial-aux row" path or an "all-occluded batch" path. | Add `test_partial_aux_row_loads` (dataset-side), `test_resolve_head_mask_defaults_for_missing_keys` (framework helper directly), and `test_all_occluded_batch_collator_invariant` (collator precondition the patch relies on). All in §3.4. |
...
```python
# Same fix in visualize_batch() (replaces the equivalent fallback near line 654):
# (the dispatch shape differs but the mask-resolution step is identical)
for name, head in self.aux_heads.items():
    mask = _resolve_head_mask(name, batch_dict, hidden.shape[0], hidden.device)
    head.visualize(hidden, batch_dict, mask=mask, ...)  # remaining args unchanged
```

The visualize call sites in `recon_head.py:179` and `pose_head.py:367` already short-circuit when `not mask.any()` (they check `valid_indices = mask.nonzero(...)` and return early if empty). Routing the visualize-path mask through `_resolve_head_mask` with the all-False default for non-universal heads makes those short-circuits actually fire instead of letting the heads crash on missing batch keys.
...
**Pass condition:** 2 global steps complete, finite total loss. Per-head loss metrics will always be present (the `_resolve_head_mask` patch routes all-occluded batches into the head's `not mask.any()` early-exit which returns `get_dummy_loss()` — that's a 0-valued tensor, not a missing key). What's expected: when a step's batch has every sample missing an aux field, the per-head loss reads as ≈ 0 / dummy and the visualization for that head no-ops for that step. With per_device_batch_size=2 and ~12.5% per-frame occlusion, P(both samples occlude the same field) ≈ 1.5%, so a 2-step smoke might or might not surface this; it does, however, surface any general crash from the framework patch.
...
| **R5**: A batch where every sample lacks a given aux field. | Without §3.3, recon/pose heads crash on `batch[field]` access (KeyError) because the collator omits both the field and its mask. | §3.3 framework patch supplies an all-False default mask for non-universal heads at both `forward()` and `visualize_batch()` call sites; the head's existing `not mask.any()` early-exit returns `get_dummy_loss()` (forward) or no-ops (visualize), preserving DeepSpeed ZeRO-2 all-reduce shape across ranks. §3.4 `test_resolve_head_mask_defaults_for_missing_keys` exercises the helper directly; `test_all_occluded_batch_collator_invariant` covers the precondition (collator drops the field+mask key when every sample lacks it). |
