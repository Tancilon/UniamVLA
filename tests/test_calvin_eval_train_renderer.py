...
def test_calvin_policy_client_normalizes_auxvla_gr00t_robot_state(monkeypatch, tmp_path):
    eval_calvin = _load_eval_calvin(monkeypatch)

    class AuxVLAGR00TModelClient(_FakeModelClient):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.unnorm_key = "franka"
            self.model_config = {
    ...
    monkeypatch.setattr(eval_calvin, "ModelClient", AuxVLAGR00TModelClient)
