#!/usr/bin/env bash
# UamGR00T_LT — joint latent-token depth and future supervision on CALVIN ABC.
#
# Requires depth and future sidecars in every A/B/C dataset. For each scene:
...
CONFIG_YAML="${CONFIG_YAML:-./examples/calvin/train_files/run_uamgr00t_LT_depth_future_train.yaml}"
NUM_GPUS="${NUM_GPUS:-4}"
...
RUN_ID="${RUN_ID:-uamvla_gr00t_lt_calvin_abc_depth_future}"
