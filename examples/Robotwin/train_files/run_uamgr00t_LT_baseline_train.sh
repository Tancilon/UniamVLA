#!/usr/bin/env bash
# UamGR00T_LT baseline for RoboTwin2.
#
# No auxiliary sidecars are required. CLI overrides pass through "$@", e.g.
...
CONFIG_YAML="${CONFIG_YAML:-./examples/Robotwin/train_files/run_uamgr00t_LT_baseline_train.yaml}"
NUM_GPUS="${NUM_GPUS:-4}"
...
RUN_ID="${RUN_ID:-uamvla_gr00t_lt_robotwin_baseline}"
